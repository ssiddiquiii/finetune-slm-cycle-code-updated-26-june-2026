# ============================================================
# CELL 1 — DEPENDENCY INSTALLATION & ENVIRONMENT BOOTSTRAP
...
# ● DEPLOYS EVALUATION STACK: DEEPEVAL, LITELLM, NEST_ASYNCIO
...
# ● IMPORTS UNSLOTH FIRST TO PATCH TRITON ATTENTION KERNELS
# ● VERIFIES ALL LIBRARY VERSIONS: TORCH, TRANSFORMERS, PEFT, TRL
# ● SCANS CUDA DEVICE COUNT, VRAM CAPACITY AND COMPUTE CAPABILITY
# ● CONFIRMS TARGET T4 GPU INFRASTRUCTURE IS ACTIVE AND READY
# ● VALIDATES KAGGLE SECRET MOUNTS FOR HF AND GROQ CREDENTIALS
# ============================================================
...
# ● CONFIRMS GROQ API BRIDGE IS LIVE FOR JUDGE MODEL REQUESTS
# ============================================================
...
# ============================================================
# CELL 4 — GLOBAL PIPELINE CONFIGURATION & DIRECTORY SETUP
# ● DEFINES BASE MODEL ID, DATASET REPO AND EVAL SPLIT TARGETS
# ● SETS INFERENCE PARAMS: MAX_NEW_TOKENS AND MAX_INPUT_LENGTH
# ● CONFIGURES JUDGE MODEL (GPT-OSS-20B) AND PASS THRESHOLD (0.7)
# ● CREATES LOCAL RESULT AND LOG DIRECTORIES IN KAGGLE WORKSPACE
# ● REGISTERS OUTPUT FILE PATHS FOR BASELINE ANSWERS AND SCORES
# ============================================================
...
# ============================================================
# CELL 5 — GOLDEN EVAL DATASET STREAMING FROM HUGGINGFACE HUB
# ● STREAMS GOLDEN_EVAL SPLIT DIRECTLY FROM HF PARQUET STORE
# ● LOADS INTO VOLATILE RAM WITHOUT POLLUTING LOCAL DISK CACHE
# ● APPLIES REPRODUCIBLE SEED=42 FOR CONSISTENT SAMPLE SELECTION
# ● DRAWS EXACTLY 54 SAMPLES: 6 INSTANCES × 9 REPAIR CATEGORIES
# ● VERIFIES FEATURE KEYS PRESENT IN THE LOADED DATASET SLICE
# ============================================================
...
# ============================================================
# CELL 6 — DYNAMIC SCHEMA FIELD DETECTION & METRIC SELECTION
# ● AUTO-DETECTS QUESTION, ANSWER AND CONTEXT FIELD NAMES
# ● SUPPORTS MULTIPLE KEY ALIASES: QUERY, INPUT, PROMPT ETC.
# ● VALIDATES CONTEXT COVERAGE ACROSS 80% OF DATASET SAMPLES
# ● CONDITIONALLY ACTIVATES FAITHFULNESS AND CONTEXTUAL METRICS
# ● ASSEMBLES FINAL EVALUATION METRIC VECTOR FOR JUDGE PIPELINE
# ============================================================
...
# ============================================================
# CELL 7 — LLM-AS-A-JUDGE SETUP WITH RATE LIMIT THROTTLING
# ● INSTANTIATES THREAD-SAFE LITELLM JUDGE CLIENT (GPT-OSS-20B)
# ● IMPLEMENTS RPM AND TPM RATE LIMITING TO PREVENT 429 ERRORS
# ● CONFIGURES ANSWER RELEVANCY METRIC WITH 0.7 PASS THRESHOLD
# ● SETS UP GEVAL CORRECTNESS FOR TECHNICAL CAR REPAIR DOMAIN
# ● CONDITIONALLY MOUNTS FAITHFULNESS AND CONTEXTUAL RELEVANCY
# ============================================================
...
judge = LiteLLMJudge(model_name=CONFIG.eval_model, api_key=GROQ_API_KEY, rpm_limit=120, tpm_limit=200000, max_tokens=1024)
...
# ============================================================
# CELL 8 — BASE MODEL LOADING IN 4-BIT NF4 QUANTIZED MODE
...
# ● REPORTS PER-GPU VRAM USAGE AFTER MODEL IS FULLY RESIDENT
# ============================================================
...
# ============================================================
# CELL 9 — BASELINE INFERENCE GENERATION (PRE-FINETUNE MODEL)
# ● CONSTRUCTS GEMMA-4 CHAT TEMPLATE WITH MERGED SYSTEM PROMPT
# ● RUNS DETERMINISTIC GREEDY DECODING (DO_SAMPLE=FALSE)
# ● ITERATES ALL 54 EVAL SAMPLES WITH TQDM PROGRESS TRACKING
# ● CAPTURES QUESTION, EXPECTED AND GENERATED ANSWER TRIPLES
# ● CACHES BASELINE ANSWERS TO JSON FILE FOR STAGE 3 COMPARISON
# ============================================================
...
def build_prompt(question: str) -> str:
    # GEMMA-4 STRUCTURAL FIX: Gemma-4 has no native system role in its chat template.
    # Merging system prompt into user turn guarantees train/infer format alignment.
    # A format mismatch here = artificially low eval scores on a model that is actually better.
    messages = [
...
# ============================================================
# CELL 10 — AUTOMATED LLM JUDGE SCORING WITH CHECKPOINT RESUME
# ● EVACUATES BASE MODEL FROM VRAM TO FREE GPU FOR API CALLS
# ● BUILDS DEEPEVAL TEST CASES FROM BASELINE ANSWER TRIPLES
# ● SCORES EACH CASE WITH ALL ACTIVE METRICS SEQUENTIALLY
# ● WRITES INCREMENTAL JSONL CHECKPOINT FOR CRASH RECOVERY
# ● COMPUTES MEAN SCORE AND PASS RATE PER METRIC IN SUMMARY
# ============================================================
...
# ============================================================
# CELL 11 — ARTIFACT PACKAGING & KAGGLE DATASET EXPORT
# ● ZIPS ALL RESULT JSON FILES INTO A SINGLE PORTABLE ARCHIVE
# ● MAKES ARTIFACTS DOWNLOADABLE FROM KAGGLE OUTPUT SIDEBAR
# ● REPORTS FINAL ZIP SIZE FOR STORAGE BUDGET MONITORING
# ● PROVIDES STEP-BY-STEP INSTRUCTIONS FOR KAGGLE DATASET MOUNT
# ● PREPARES BASELINE ARTIFACTS AS INPUT FOR STAGE 3 POST-EVAL
# ============================================================
